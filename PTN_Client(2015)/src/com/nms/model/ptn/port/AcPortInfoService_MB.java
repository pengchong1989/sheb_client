...
public class AcPortInfoService_MB extends ObjectService_Mybatis{
	...
	public void getAcQos(List<AcPortInfo> acPortInfoList) {
	 ...
	 * @throws Exception
	 */
	public void getAcBuffer(List<AcPortInfo> acPortInfoList) throws Exception {
...
//			UiUtil.closeService(bufservice);
	...
	public List<Integer> acByPort(int portId, int siteId) {
		List<Integer> idList = new ArrayList<Integer>();
		try {
			idList = this.mapper.selectByPortId(portId);
			PortLagInfoMapper lagMapper = this.sqlSession.getMapper(PortLagInfoMapper.class);
			PortInstMapper portMapper = this.sqlSession.getMapper(PortInstMapper.class);
			PortInst port = portMapper.selectPortybyid(portId);
			PortLagInfo condition = new PortLagInfo();
			if(siteId == 0){
				siteId = port.getSiteId();
			}
			condition.setSiteId(siteId);
			List<PortLagInfo> lagInfoList = lagMapper.queryByCondition(condition);
			if(lagInfoList != null && lagInfoList.size() > 0){
				if(idList == null){
					idList = new ArrayList<Integer>();
				}
				for (PortLagInfo portLagInfo : lagInfoList) {
					if(portLagInfo.getPortLagMember().contains(port.getNumber()+"")){
						AcPortInfo acCondition = new AcPortInfo();
						acCondition.setLagId(portLagInfo.getId());
						List<AcPortInfo> acList = this.mapper.queryByCondition(acCondition);
...
//			UiUtil.closeService(qosRelevanceService);
	 ...
	 * @throws Exception
	 */
	public List<AcPortInfo> selectBySiteIdOnly(AcPortInfo acInfo) throws Exception {
		List<AcPortInfo> acPortInfoList = null;
		try {
			acPortInfoList = this.mapper.queryByCondition(acInfo);
		} catch (Exception e) {
